...
class MyFrame(ctk.CTkScrollableFrame):
    def __init__(self, master, **kwargs):
        ...
        self.buttons_frame.grid(row=0, column=2, padx=10, pady=10, sticky=ctk.E)

        # CTkButton to fetch data
        self.fetch_button = ctk.CTkButton(self.buttons_frame, text="Traer Datos", command=self.fetch_data)
        self.fetch_button.grid(row=0, column=0, padx=10, pady=10)

        # CTkButton to update data
        self.update_button = ctk.CTkButton(self.buttons_frame, text="Actualizar Datos", command=self.update_data, state=ctk.DISABLED, fg_color="green")
        self.update_button.grid(row=0, column=1, padx=10, pady=10)

        # CTkButton to reset interface
        self.cancel_button = ctk.CTkButton(self.buttons_frame, text="Cancelar", command=self.reset_interface, fg_color="red")
    ...
    def fetch_data(self):
        try:
            ...
            self.column_names.extend([row.COLUMN_NAME for row in cursor.fetchall()])
            ...
            primary_key_value = int(self.entry_id.get())  # Get the ID entered by the user
            ...
            if result:
                ...
                for i, custom_label in enumerate(self.custom_labels):
                    ...
                    if custom_label == 'Area que solicita':
                        ...
                        combobox = ctk.CTkComboBox(self, values=area_values, state='readonly')
                        ...
                        combobox = ctk.CTkComboBox(self, values=fraccion_values, state='readonly')
                        combobox.set(result[i]) if result[i] is not None else ""
                        combobox.grid(row=i, column=1, padx=5, pady=5, sticky=ctk.W)
                        self.data_entries.append(combobox)
                        ...
                        combobox = ctk.CTkComboBox(self, values=proveedor_values, state='readonly')
                        combobox.set(result[i]) if result[i] is not None else ""
                        combobox.grid(row=i, column=1, padx=5, pady=5, sticky=ctk.W)
                        self.data_entries.append(combobox)
                        ...
                        # Use Combobox for specific entry fields
                        common_values = ["SI", "NO"]
                        combobox = ctk.CTkComboBox(self, values=common_values, state='readonly')
                        combobox.set(result[i]) if result[i] is not None else ""
                        combobox.grid(row=i, column=1, padx=5, pady=5, sticky=ctk.W)
                        self.data_entries.append(combobox)
                ...
                self.warning_label.configure(text="ID no encontrado, Porfavor ingrese un ID valido")
    ...
    def select_date(self,label):
        ...
        set_date_button = ctk.CTkButton(date_picker, text='Fijar fecha', command=lambda: self.set_selected_date(date_entry, label))
    ...
    def set_selected_date(self,date_entry, label):
        ...
        self.data_entries[entry_index].insert(0, selected_date.strftime('%d/%m/%Y'))
    ...
    def update_data(self):
        try:
            ...
            set_clause = ", ".join(f"{column_name} = ?" for column_name in self.column_names if column_name != 'ID')

            # Get values from the entry fields (excluding ID value)
            update_values = [entry.get() for entry in self.data_entries[1:]]  # Exclude the ID entry
    ...
    # Disable the date entry buttons
        for button in self.select_date_buttons:
            button.configure(state=ctk.DISABLED)
        # Enable the "Traer Datos" button
        self.fetch_button.configure(state=ctk.NORMAL)# Enable the ID entry for editing
